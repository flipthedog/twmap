...
# Function to process a single world for players
process_world() {
    local WORLD_ID=$1
    ...
    if [ $PLAYER_FILE_COUNT -eq 0 ]; then
        echo "No player maps found for world ${WORLD_ID}. Skipping..."
        return 1
    fi

    echo "Found ${PLAYER_FILE_COUNT} player maps for world ${WORLD_ID}."
    ...
    # Process files
    echo "Generating player timelapse for world ${WORLD_ID}..."
    ...
    if [ $? -eq 0 ]; then
        ...
        FINAL_IMAGE=$(ls -1 ${BASE_DIR}/players/*.png | tail -1)
        if [ -n "$FINAL_IMAGE" ]; then
            cp "$FINAL_IMAGE" ${OUTPUT_DIR}/${WORLD_ID}_player_final_frame.png
            if [ $? -eq 0 ]; then
...
echo "===== Player Timelapse Generator ====="
echo ""
...
echo "Player MP4 files are in the 'outputs/' directory"
