...
process_world() {
    ...
    if [ -n "$(ls -A ${BASE_DIR}/players/ 2>/dev/null)" ]; then
        ...
        ffmpeg -thread_queue_size 4096 -framerate ${PLAYER_FRAMERATE} -i ${BASE_DIR}/players/%d.png -crf 16 -vf "minterpolate='mi_mode=mci:mc_mode=aobmc:vsbmc=1:fps=${PLAYER_FRAMERATE}',hqdn3d" -pix_fmt yuv420p ${OUTPUT_DIR}/${WORLD_ID}_player_output.mp4 -y -loglevel error
    ...
    if [ -n "$(ls -A ${BASE_DIR}/tribes/ 2>/dev/null)" ]; then
        ...
        ffmpeg -thread_queue_size 4096 -framerate ${TRIBE_FRAMERATE} -i ${BASE_DIR}/tribes/%d.png -crf 16 -vf "minterpolate='mi_mode=mci:mc_mode=aobmc:vsbmc=1:fps=${TRIBE_FRAMERATE}',hqdn3d" -pix_fmt yuv420p ${OUTPUT_DIR}/${WORLD_ID}_tribe_output.mp4 -y -loglevel error
